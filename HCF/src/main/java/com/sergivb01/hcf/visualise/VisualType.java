...

		{
			this.blockFiller = new BlockFiller(){
				...
				VisualBlockData generate(final Player player, final Location location){
					return new VisualBlockData(Material.STAINED_GLASS, DyeColor.MAGENTA.getData());
				}
			};
		}
...

		{
			this.blockFiller = new BlockFiller(){
				...
				VisualBlockData generate(final Player player, final Location location){
					...
					final Faction faction = HCF.getInstance().getFactionManager().getFactionAt(location);
