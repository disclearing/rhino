...
public class TimerSidebarProvider implements SidebarProvider{
	...
	public List<SidebarEntry> getLines(final Player player){
		...
		final SaleTimer.SaleRunnable saleRunnable = this.plugin.getSaleTimer().getSaleRunnable();
		final KeyAllTimer.KeyAllRunnable keyAllRunnable = this.plugin.getKeyAllTimer().getkeyAllRunnable();

		if(sotwRunnable != null && !sotwRunnable.isSotwEnabled(player)){
		...
		} else if(sotwRunnable != null){
			lines.add(new SidebarEntry(ChatColor.GREEN.toString() + ChatColor.BOLD, "SOTW" + ChatColor.DARK_GRAY + ": ", ChatColor.GRAY + "(PvP Enabled)"));
		}

		if(keySaleRunnable != null) {
			lines.add(new SidebarEntry(ChatColor.LIGHT_PURPLE.toString() + ChatColor.BOLD, "Key Sale" + ChatColor.DARK_GRAY + ": ", ChatColor.RED + DurationFormatter.getRemaining(keySaleRunnable.getRemaining(), true)));
		}

		if(saleRunnable != null) {
			lines.add(new SidebarEntry(ChatColor.AQUA.toString() + ChatColor.BOLD, "Sale" + ChatColor.DARK_GRAY + ": ", ChatColor.RED + DurationFormatter.getRemaining(saleRunnable.getRemaining(), true)));
		}

		if(keyAllRunnable != null) {
			lines.add(new SidebarEntry(ChatColor.YELLOW.toString() + ChatColor.BOLD, "Key All" + ChatColor.DARK_GRAY + ": ", ChatColor.RED + DurationFormatter.getRemaining(keyAllRunnable.getRemaining(), true)));
		}

		if((pvpClass instanceof MinerClass)){
			lines.add(new SidebarEntry(ChatColor.GRAY.toString() + ConfigurationService.DOUBLE_ARROW + " ", ChatColor.AQUA + "Diamonds", ChatColor.DARK_GRAY + ": " + ChatColor.RED + player.getStatistic(Statistic.MINE_BLOCK, Material.DIAMOND_ORE)));
		}
...
*/
		...
		if(baseUser.isVanished() && !baseUser.isStaffUtil()){
			lines.add(new SidebarEntry(" " + ChatColor.GRAY.toString() + ConfigurationService.DOUBLE_ARROW + " " + ChatColor.YELLOW.toString(), "Vanished" + ChatColor.GRAY + ": ", baseUser.isVanished() ? (ChatColor.GREEN + "True") : (ChatColor.RED + "Visible")));
		}else if(baseUser.isStaffUtil()){
			lines.add(new SidebarEntry(ChatColor.GOLD + "" + ChatColor.BOLD + "Staff Mode: "));

			lines.add(new SidebarEntry(" " + ChatColor.GRAY.toString() + ConfigurationService.DOUBLE_ARROW + " " + ChatColor.YELLOW.toString(), "Vanished" + ChatColor.GRAY + ": ", baseUser.isVanished() ? (ChatColor.GREEN + "True") : (ChatColor.RED + "Visible")));
			lines.add(new SidebarEntry(" " + ChatColor.GRAY.toString() + ConfigurationService.DOUBLE_ARROW + " " + ChatColor.YELLOW.toString(), "Channel" + ChatColor.GRAY + ": ", baseUser.isInStaffChat() ? (ChatColor.AQUA + "Staff Chat") : (ChatColor.GREEN + "Global")));

			if(BasePlugin.getPlugin().getServerHandler().isChatDisabled()){
				lines.add(new SidebarEntry(" " +ChatColor.GRAY.toString() + ConfigurationService.DOUBLE_ARROW + " " + ChatColor.YELLOW.toString() + "Chat", ChatColor.GRAY + ":" + ChatColor.WHITE + " Locked ", "(" + HCF.getRemaining(BasePlugin.getPlugin().getServerHandler().getChatDisabledMillis() - System.currentTimeMillis(), true) + ")"));
			}

			if(isChatSlowed()){
				lines.add(new SidebarEntry(" " +ChatColor.GRAY.toString() + ConfigurationService.DOUBLE_ARROW + " " + ChatColor.YELLOW.toString() + "Chat", ChatColor.GRAY + ":" + ChatColor.WHITE + " Slowed ", "(" + BasePlugin.getPlugin().getServerHandler().getChatSlowedDelay() + "s)"));
			}

			lines.add(new SidebarEntry(" " + ChatColor.GRAY.toString() + ConfigurationService.DOUBLE_ARROW + " " + ChatColor.YELLOW.toString() + "Players", ChatColor.GRAY + ": ", ChatColor.RED.toString() + Bukkit.getOnlinePlayers().size()));
