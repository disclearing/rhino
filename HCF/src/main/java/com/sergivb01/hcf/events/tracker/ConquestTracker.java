...
public class ConquestTracker implements EventTracker, Listener{
	...
	public void onContest(final EventFaction eventFaction, final EventTimer eventTimer){
		Bukkit.broadcastMessage(ChatColor.GRAY + "[" + ChatColor.GOLD.toString() + ChatColor.BOLD.toString() + eventFaction.getName() + ChatColor.GRAY + "] " + ChatColor.GOLD + eventFaction.getName() + " §ecan now be contested.");
	...
	public boolean onControlLoss(final Player player, final CaptureZone captureZone, final EventFaction eventFaction){
		...
		if(remainingMillis > 0L && captureZone.getDefaultCaptureMillis() - remainingMillis > ConquestTracker.MINIMUM_CONTROL_TIME_ANNOUNCE){
			Bukkit.broadcastMessage(ChatColor.GRAY + "[" + ChatColor.GOLD.toString() + ChatColor.BOLD.toString() + eventFaction.getName() + ChatColor.GRAY + "] " + ChatColor.GOLD + player.getName() + " §ewas knocked off " + captureZone.getDisplayName() + ChatColor.YELLOW + '.');
		}
		return true;
	}
		...
		if(currentEventFac instanceof ConquestFaction){
			...
			if(!ConfigurationService.KIT_MAP){
				if(playerFaction != null){
					...
					event.setDeathMessage(ChatColor.GRAY + "[" + ChatColor.GOLD.toString() + ChatColor.BOLD.toString() + currentEventFac.getName() + ChatColor.GRAY + "] " + ChatColor.LIGHT_PURPLE + playerFaction.getName() + ChatColor.YELLOW + " lost " + ChatColor.RED + Math.min(20, oldPoints) + ChatColor.YELLOW + " points because " + player.getName() + " died." + ChatColor.RED + " (" + this.getPoints(playerFaction) + '/' + 300 + ')' + ChatColor.YELLOW + '.');
				}
			}
		}
	}
