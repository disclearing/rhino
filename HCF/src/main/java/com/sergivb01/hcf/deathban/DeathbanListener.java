...
import com.sergivb01.hcf.utils.ItemBuilder;
...
public class DeathbanListener
	...
	public void onRespawn(PlayerRespawnEvent event) {
		if(ConfigurationService.DEATHBAN_ROOM) {
			...
			String deathbanWorld = plugin.getConfig().getString("deathban_room.world_name");
			String display = ChatColor.translateAlternateColorCodes('&', plugin.getConfig().getString("deathban_room.information_name"));
			String display1 = ChatColor.translateAlternateColorCodes('&', plugin.getConfig().getString("deathban_room.use_lives_name"));
			String display2 = ChatColor.translateAlternateColorCodes('&', plugin.getConfig().getString("deathban_room.teleport_spawn_name"));
			...
			ItemStack builder = new com.sergivb01.util.ItemBuilder(mat).displayName(display).build();
			ItemStack builder1 = new com.sergivb01.util.ItemBuilder(mat1).displayName(display1).build();
			ItemStack builder2 = new com.sergivb01.util.ItemBuilder(mat2).displayName(display2).build();
			player.getInventory().clear();
			player.getInventory().setItem(plugin.getConfig().getInt("deathban_room.information_slot"), builder);
			player.getInventory().setItem(plugin.getConfig().getInt("deathban_room.use_lives_slot"), builder1);
			player.getInventory().setItem(plugin.getConfig().getInt("deathban_room.teleport_spawn_slot"), builder2);
			new BukkitRunnable() {
				@Override
				public void run() {
					player.teleport(new Location((World) Bukkit.getWorld(deathbanWorld),
							(double) plugin.getConfig().getDouble("deathban_room.x"),
							(double) (plugin.getConfig().getDouble("deathban_room.y") + 2), (double) plugin.getConfig().getDouble("deathban_room.z"), (float) plugin.getConfig().getFloat("deathban_room.yaw"), (float) plugin.getConfig().getFloat("deathban_room.pitch")));
					player.sendMessage(ChatColor.GREEN + "You've been sent to the hospital!");
				}
